                ...
                if c < 400 and c > 395:
                    if self.thrust > self.THRUST_WARNING:
                        if self.equipment[DOCKING_COMPUTER] == 0: 
                                ...
                                if(flatland_engine.tutorial['FIRST_FLIGHT_2'] == False):
                                    i = 15
                                    self.add_hud_message( \
                                            'Reduce speed before docking!', i, \
                                             flatland_engine.tutorial_color, False)
                                    self.add_hud_message( \
                                            'Use DOWN arrow key', i+2, \
                                             flatland_engine.tutorial_color, False)
                                    flatland_engine.sound.play_sound_effect('SUCCESS')
                                    flatland_engine.tutorial['FIRST_FLIGHT_2'] = True
                        else: 
    ...
    def build_draw_group(self):
        ...
        if self.player_ship.state != define.ABANDONED:
            ...
            if self.reward_given == False:
                ...
                # If this is a Thargoid controlled system...
                if self.planet_data.num_thargoids > 0:

                    # If all (actually most) Thargoids have been destroyed...
                    if self.planet_data.num_thargoids < 4:
                        ...
                        self.player_ship.add_direct_bounty(25000)

                        self.player_ship.add_hud_message( \
                               '25,000 Cr REWARD RECEIVED', \
                ...
                # not under Thargoid control...
                elif self.planet_data.government < 7:

                    # reward for clearing all pirates out of any system that
                    # is not a Corporate State (since 0 pirates anyway)
                    # and that is not under thargoid control
                    n_pir = 0
                    for i in self.all_objects_group:
                        if isinstance(i, objects.Pirate) == True:
                            n_pir += 1
                    if n_pir == 0:

                        name = self.planet_data.name
                        self.player_ship.add_hud_message( \
                               name.upper() + ' SYSTEM HAS BEEN CLEARED OF PIRATES!', \
                               16, define.PINK, False)
                        
                        self.player_ship.add_direct_bounty(4500)

                        self.player_ship.add_hud_message( \
                               '4,500 Cr REWARD RECEIVED', \
                               18, define.ORANGE, False)
